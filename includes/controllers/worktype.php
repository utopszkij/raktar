...
include_once __DIR__.'/../models/worktypemodel.php';

/**
 * worktype controller 
 * igényelt model (includes/models/worktypemodel.php))
 *      methodusok: emptyRecord(), save($record), 
 *      getById($id), deleteById($id), getItems($page,$limit,$filter,$order), 
 *      getTotal($filter)
 * igényelt viewerek includes/views/worktypebrowser, includes/views/worktypeform 
 *      a worktypeform legyen alkalmas show funkcióra is a record, loged, logedAdmin -tól függően
...
class Worktype extends Controller {
	...
	function __construct() {
		parent::__construct();
		// $this->model = new WorktypeModel();
        $this->name = "worktype";
        $this->browserURL = 'index.php?task=worktype.items';
        $this->addURL = 'index.php?task=worktype.new';
        $this->editURL = 'index.php?task=worktype.edit';
        $this->browserTask = 'worktype.items';
        $this->model = new WorktypeModel();
		$this->ckeditorFields = [];
	}
